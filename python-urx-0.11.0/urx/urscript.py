...
class URScript(object):
    ...
    def _socket_get_var(self, var, socket_name):
        msg = "{} = socket_get_var(\"{}\",\"{}\")".format(var, var, socket_name)
        self.add_line_to_program(msg)
        self._sync()

    def _socket_send_int(self, value, socket_name):
        msg = "socket_send_byte(\"{}\",\"{}\")".format(value, socket_name)
        self.add_line_to_program(msg)
        self._sync()
    ...
    def _textmsg(self, s1, s2=''):
        msg = "textmsg(\"{}\",\"{}\")".format(s1, s2)
        self.add_line_to_program(msg)

    def _get_grip_opening(self,var_name):
        msg = "pos = socket_get_var(\"POS\",\"gripper_socket\")"
        self.add_line_to_program(msg)
        self._sync()

    def _send_var(self, var_name, socket_name):
        self._socket_set_var("POSG", var_name, socket_name)
